...
(also talked about [[02 - cryptographic concepts#hash functions|here]])
...
`C_t` and the secret key (which client and server share) are then used in the HMAC ([[02 - cryptographic concepts#MAC with one-way hash functions|hash-based MAc]]) to get an authentication tag 
